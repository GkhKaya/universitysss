import type { AppMessages } from '../types'

export const en: AppMessages = {
  home: {
    title: 'universitysss',
    description:
      'A Q&A app for university life. We are still building content and the interface.',
    registerCta: 'Sign up',
    loginCta: 'Log in',
  },
  register: {
    heroTitle: 'Meet in the light of knowledge.',
    heroSubtitle:
      'Answers that make campus life easier—curated with care.',
    heroImageAlt: 'Dumlupınar University Faculty of Engineering logo',
    formTitle: 'Join us',
    formSubtitle: 'You are only a few steps away from your place in the academic world.',
    roleLabel: 'Role',
    roleStudent: 'Student',
    roleTeacher: 'Teacher',
    fullNameLabel: 'Full name',
    fullNamePlaceholder: 'e.g. Jane Doe',
    emailLabel: 'University email',
    emailPlaceholder: 'name@ogr.dpu.edu.tr or name@dpu.edu.tr',
    departmentLabel: 'Department',
    departmentPlaceholder: 'Pick from list or search…',
    departmentLoading: 'Loading departments…',
    departmentLoadError: 'Could not load departments. Refresh the page.',
    departmentEmpty: 'No departments yet. An admin must add them in Firestore.',
    departmentNoMatch: 'No department matches your search.',
    passwordLabel: 'Password',
    passwordPlaceholder: '••••••••',
    passwordShowAria: 'Show password',
    passwordHideAria: 'Hide password',
    termsPrefix: 'By signing up you accept the ',
    termsLink: 'Terms of Use',
    termsMiddle: ' and the ',
    privacyLink: 'Privacy Policy',
    termsSuffix: '.',
    submitButton: 'Create account',
    submitLoading: 'Saving…',
    footerPrompt: 'Already have an account?',
    loginLink: 'Log in',
    localeSwitchLabel: 'Language',
    errorRequired: 'Please fill in all fields.',
    errorDepartmentRequired: 'Please select a department from the list.',
    errorPasswordLength: 'Password must be at least 6 characters.',
    errorEmailStudentDomain:
      'Student registration only accepts addresses that are exactly @ogr.dpu.edu.tr (plain @dpu.edu.tr is not allowed).',
    errorEmailTeacherDomain:
      'Faculty registration only accepts addresses that are exactly @dpu.edu.tr (@ogr.dpu.edu.tr is not allowed).',
    errorSignup: 'Sign-up failed. Check your details.',
    errorRoleNotFound:
      'The selected role does not match Firestore. Check userRoles documents in the admin console.',
    errorDepartmentNotFound:
      'Department not found. Type the full name or a substring that matches a department record.',
    errorProfileWrite: 'Account was created but the profile document could not be saved. Contact support.',
    successSignup: 'Your account was created. Redirecting to sign-in…',
  },
  login: {
    heroTitle: 'Welcome back.',
    heroSubtitle: 'Continue with your institutional account.',
    heroImageAlt: 'Dumlupınar University Faculty of Engineering logo',
    formTitle: 'Sign in',
    formSubtitle: 'Use your university email and password.',
    emailLabel: 'University email',
    emailPlaceholder: 'name@ogr.dpu.edu.tr or name@dpu.edu.tr',
    passwordLabel: 'Password',
    passwordPlaceholder: '••••••••',
    passwordShowAria: 'Show password',
    passwordHideAria: 'Hide password',
    submitButton: 'Sign in',
    submitLoading: 'Signing in…',
    footerPrompt: 'No account yet?',
    registerLink: 'Sign up',
    localeSwitchLabel: 'Language',
    errorRequired: 'Email and password are required.',
    errorInvalidCredentials: 'Incorrect email or password.',
    errorLoginFailed: 'Sign-in failed. Please try again.',
    successLogin: 'Signed in. Redirecting…',
  },
  ask: {
    pageTitle: 'Ask a Question',
    pageSubtitle: 'Write a clear title and pick the right category for faster answers.',
    searchPlaceholder: 'Search questions or ask a new one...',
    menuHome: 'Home',
    menuCategories: 'Categories',
    menuScholarships: 'Scholarships',
    menuRegistration: 'Registration',
    menuMyQuestions: 'My Questions',
    menuAsk: 'Ask Question',
    titleLabel: 'Question title',
    titlePlaceholder: 'Summarize your question briefly...',
    departmentLabel: 'Department',
    departmentPlaceholder: 'Select a department...',
    departmentLoading: 'Loading departments...',
    departmentLoadError: 'Could not load departments. Please refresh.',
    categoryLabel: 'Category',
    categoryPlaceholder: 'Select a category...',
    categoryLoading: 'Loading categories...',
    categoryLoadError: 'Could not load categories. Please refresh.',
    contentLabel: 'Details',
    contentPlaceholder: 'Explain your question and what you have tried...',
    anonymousLabel: 'Post anonymously',
    targetAudienceLabel: 'Target audience',
    targetAudienceEveryone: 'Everyone',
    targetAudienceToStudent: 'Ask students',
    targetAudienceToTeacher: 'Ask teachers',
    submitButton: 'Submit Question',
    submitLoading: 'Submitting...',
    cancelButton: 'Cancel',
    successTitle: 'Question received',
    successDescription: 'Your question will be published after review. Thank you.',
    successPrimaryAction: 'Ask Another',
    successSecondaryAction: 'Back to Home',
    tipsTitle: 'Tips',
    tipsItems: [
      'Use a short and clear title.',
      'Add what steps you already tried.',
      'Choosing the right category improves visibility.',
    ],
    sampleCategoriesTitle: 'Sample Categories',
    errorRequired: 'Please fill in title and details.',
    errorCategoryRequired: 'Please select a category.',
    errorTitleTooLong: 'Title can be at most 120 characters.',
    errorContentTooShort: 'Details must be at least 20 characters.',
    errorUnauthenticated: 'Please sign in again to submit a question.',
    errorProfileNotFound: 'Your user profile was not found. Contact support.',
    errorDepartmentRequired: 'Please select a department.',
    errorDepartmentNotFound: 'Selected department was not found. Refresh and try again.',
    errorCategoryNotFound: 'Selected category was not found. Refresh and try again.',
    errorApprovalForbidden: 'This action requires teacher or admin access.',
    errorApprovalUpdateFailed: 'Could not save question approval. Please try again.',
    errorCreateFailed: 'Question could not be submitted. Please try again.',
  },
  myQuestions: {
    pageTitle: 'My Questions',
    pageSubtitle: 'Track all the questions you have submitted and their current status.',
    searchPlaceholder: 'Search your questions...',
    menuHome: 'Home',
    menuCategories: 'Categories',
    menuScholarships: 'Scholarships',
    menuRegistration: 'Registration',
    menuMyQuestions: 'My Questions',
    menuAsk: 'Ask Question',
    emptyTitle: 'No questions yet',
    emptySubtitle: 'Click the button below to ask your first question.',
    askButton: '+ Ask a Question',
    statusAll: 'All',
    statusPending: 'Pending',
    statusAnswered: 'Answered',
    questionAnswers: 'Answers',
    questionLikes: 'Likes',
    questionPending: 'Pending',
    questionAnswered: 'Answered',
  },
  questionApprovals: {
    pageTitle: 'Question Approvals',
    pageSubtitle: 'Review pending questions and publish them.',
    menuHome: 'Home',
    menuMyQuestions: 'My Questions',
    menuAsk: 'Ask Question',
    menuApprovals: 'Question Approvals',
    emptyTitle: 'No pending questions',
    emptySubtitle: 'All submitted questions have been reviewed.',
    approveButton: 'Approve',
    approvingButton: 'Approving...',
    successApproved: 'Question approved.',
    accessDenied: 'Only teacher and admin users can view this page.',
    goHome: 'Back to Home',
    genericError: 'Could not load approvals. Please refresh.',
  },
}
